...
- **消息操作**：长按菜单支持回复、转发、复制、撤回、删除、重发（危险操作二次确认）。
- **会话能力**：会话设置（置顶 / 免打扰）、群成员列表、本地消息搜索、输入草稿。
- **设置**：交互开关、退出确认、清除草稿、关于页。
- **其它**：表情资源桥接、APNG/Lottie 富图 native 初始化、崩溃捕获页、登录后进程重启以获得干净的内核初始化。

尚未完成：群聊 chatType/peerUid 映射的真机验证、语音、多选、QQ 空间、音视频通话，以及富媒体收发的服务端联调。
...
- **回复 / 转发**：长按消息可回复（输入区上方回复条）与转发（选人页）。
